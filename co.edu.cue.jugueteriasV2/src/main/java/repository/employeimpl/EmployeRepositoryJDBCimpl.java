...
public class EmployeRepositoryJDBCimpl implements Repository<employees> {
    ...
    private employees createEmployee(ResultSet resultSet) throws SQLException{
        ...
        e.setId(resultSet.getInt("id_employee"));
        e.setUser(resultSet.getString("user_employee"));
        e.setPassword(resultSet.getString("password_employee"));
        e.setEmployment_start_date(resultSet.getDate("employee_start_date"));
        return e;
    ...
    public void save(employees employees) {
        try (PreparedStatement pst = getConnection()
            ...
            pst.setDate(3,employees.getEmployment_start_date());
    ...
    public void update(employees employees) {
        try(PreparedStatement preparedStatement = getConnection()
               .prepareStatement("""
                                     UPDATE employees SET user =?, password =?,  employment_start_date =? WHERE id =?;
                                       """
                )
        ){
            preparedStatement.setString(1,employees.getUser());
            preparedStatement.setString(2,employees.getPassword());
            preparedStatement.setDate(3,employees.getEmployment_start_date());
