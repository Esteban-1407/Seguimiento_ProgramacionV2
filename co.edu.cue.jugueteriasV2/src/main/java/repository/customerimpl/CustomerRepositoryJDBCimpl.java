...
public class CustomerRepositoryJDBCimpl implements Repository<cliente> {
    ...
    public void save(cliente customer) {
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(
            ...
            preparedStatement.setString(2, customer.getIDnumber());
            preparedStatement.setDate(3, new java.sql.Date(customer.getDate_birth().getTime()));
    ...
    public void update(cliente customer) {
        try (PreparedStatement preparedStatement = getConnection().prepareStatement(
                "UPDATE cliente SET name = ?, IDnumber = ?, date_birth = ? WHERE id = ?")) {
            preparedStatement.setString(1, customer.getName());
            preparedStatement.setString(2, customer.getIDnumber());
            preparedStatement.setDate(3, new java.sql.Date(customer.getDate_birth().getTime()));
    ...
    private cliente createCustomer(ResultSet resultSet) throws SQLException {
        ...
        customer.setDate_birth(resultSet.getDate("date_birth"));
        return customer;
    }
}
